...
#define Max_No_of_CFRA_preambles 10

#include"RACH-ConfigDedicated.h"

int preamble_flagger[Max_No_of_CFRA_preambles] = {0};
...
int preamble_assigner (UE_info *UE_list )
{
	int preamble_index;
    int i;
    ...
    */
    for (i = 0; i < Max_No_of_CFRA_preambles; i++)
     ...
     preamble_index = (63 - Max_No_of_CFRA_preambles) + i;
...
int main(int argc, char const *argv[])
{
    ...
    preamble_index = preamble_assigner(UE_list);

    send(connfd,&preamble_index,sizeof(preamble_index),0); 

    recv(connfd,&PREAMBLE_INDEX,sizeof(PREAMBLE_INDEX),0);
    printf("==========1st uplink reception======================\n");
	printf("Preamble index identified from preamble is : %d\n",PREAMBLE_INDEX );
	printf("X=====================================================X\n");
